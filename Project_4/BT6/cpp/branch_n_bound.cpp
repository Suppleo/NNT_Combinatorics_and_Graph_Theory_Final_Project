...
// --- SolutionState Struct to store the best solution found so far ---
struct SolutionState {
    int cost;
    std::map<int, int> mapping;
    std::map<std::string, int> details; // Stores deletions, insertions, relabelings

    SolutionState() : cost(std::numeric_limits<int>::max()) {}
};

// --- Tree Edit Distance Functions ---

int calculate_partial_cost(Tree& T1, Tree& T2, const std::map<int, int>& M_current) {
    /*
    ...
    */
    ...
    // Deletions and Relabelings: nodes in T1 mapped to LAMBDA_NODE or different labels
    for (const auto& pair : M_current) {
        ...
        if (w_map == LAMBDA_NODE) {
            ...
            if (node_v && node_w && node_v->label != node_w->label) {
...
void branch_and_bound_extend_tree_edit(Tree& T1, Tree& T2, 
                                       ...
                                       SolutionState& best_solution) { // Reference to store the best solution found

    std::vector<TreeNode*> t1_preorder_nodes = T1.get_preorder_nodes();

    // Pruning step (Upper Bounding)
    int lower_bound_cost = calculate_partial_cost(T1, T2, M_current);
    ...
    if (current_v_idx == t1_preorder_nodes.size()) {
        ...
        auto cost_details = calculate_edit_distance(T1, T2, M_current);
        int final_cost = cost_details.first;
        
        if (final_cost < best_solution.cost) {
            best_solution.cost = final_cost;
            best_solution.mapping = M_current; // std::map copy constructor performs deep copy
            best_solution.details = cost_details.second;
        }
        return;
    }
    ...
    if (C_current.count(v_id)) { // Check if v_id still has candidates after pruning
        // Make a copy of the candidates for the current node to iterate over,
        // as `refine_candidate_nodes` might modify other candidate lists.
        std::vector<int> candidates_for_v = C_current.at(v_id); 

        for (int w_id_candidate : candidates_for_v) {
            M_current[v_id] = w_id_candidate; // Tentatively assign v_id to w_id_candidate
            ...
            // When modified via `refine_candidate_nodes`, it only affects `C_next`.
            ...
            // or implicitly remains from this branch if it's the final solution path.
        }
    }
}

SolutionState branch_and_bound_tree_edit(Tree& T1, Tree& T2) {
    // Pre-process trees to compute depths and preorder indices
    T1.compute_preorder_and_depth();
    T2.compute_preorder_and_depth();

    SolutionState best_solution; // Initialized with max cost
...
int main() {
    std::cout << "--- Example Tree Edit Distance Problem (Branch-and-Bound) ---" << std::endl;
    ...
    std::cout << "\n--- Running Branch-and-Bound Algorithm ---" << std::endl;
    
    SolutionState min_cost_solution = branch_and_bound_tree_edit(T1, T2);

    std::cout << "\n--- Minimum Edit Distance Found (Branch-and-Bound) ---" << std::endl;
    if (min_cost_solution.cost != std::numeric_limits<int>::max()) {
        ...
        std::vector<std::pair<int, int>> sorted_mapping(min_cost_solution.mapping.begin(), min_cost_solution.mapping.end());
        ...
        std::cout << "No solution found (This should not happen for a well-formed problem)." << std::endl;
    }
